<?php
namespace app\install\acl_role\make;
/*
 * Class 
 * @filename app 
 * @encoding UTF-8
 ...
 * @author Ryno
 */
class install extends \app\install\acl_role\master\main{
...
EOD;
	...
	public static function make_db_enties($options = []) {
		...
		return \app\install\acl_role\make\install::install_from_solid($options_arr["return_sql"]);
	}
	//--------------------------------------------------------------------------
	public static function install_from_db() {
		...
		foreach ($acl_role_arr as $acl_role) {
			$install = new \app\install\acl_role\make\install($acl_role->acl_name, $acl_role->acl_code, $acl_role->acl_level, $acl_role->acl_is_locked);
			$install->install();
		}
